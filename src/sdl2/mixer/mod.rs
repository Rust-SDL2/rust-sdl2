...
pub fn get_linked_version() -> Version {
    ...
    unsafe { Version::from_ll(*mixed::Mix_Linked_Version()) }
}

bitflags!(
    pub struct InitFlag : u32 {
        const FLAC = mixed::MIX_InitFlags_MIX_INIT_FLAC as u32;
        const MOD  = mixed::MIX_InitFlags_MIX_INIT_MOD as u32;
        const MP3  = mixed::MIX_InitFlags_MIX_INIT_MP3 as u32;
        const OGG  = mixed::MIX_InitFlags_MIX_INIT_OGG as u32;
        const MID  = mixed::MIX_InitFlags_MIX_INIT_MID as u32;
    }
);
...
pub fn init(flags: InitFlag) -> Result<Sdl2MixerContext, String> {
    let return_flags = unsafe {
        let ret = mixed::Mix_Init(flags.bits() as c_int);
...
pub fn open_audio(frequency: i32,
                  ...
                  -> Result<(), String> {
    let ret = unsafe {
        mixed::Mix_OpenAudio(frequency as c_int,
                           format,
                           channels as c_int,
                           chunksize as c_int)
    };
...
pub fn query_spec() -> Result<(i32, AudioFormat, i32), String> {
    ...
    let ret = unsafe { mixed::Mix_QuerySpec(&mut frequency, &mut format, &mut channels) };
    if ret == 0 {
        Err(get_error())
    } else {
...
pub fn get_chunk_decoders_number() -> i32 {
    unsafe { mixed::Mix_GetNumChunkDecoders() as i32 }
}

/// Get the name of the indexed sample chunk decoder.
pub fn get_chunk_decoder(index: i32) -> String {
    unsafe {
        let name = mixed::Mix_GetChunkDecoder(index as c_int);
...
impl Drop for Chunk {
    fn drop(&mut self) {
        if self.owned {
            unsafe { mixed::Mix_FreeChunk(self.raw) }
        }
    }
}
...
impl Chunk {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Chunk, String> {
        let raw = unsafe { mixed::Mix_LoadWAV_RW(try!(RWops::from_file(path, "rb")).raw(), 0) };
        if raw.is_null() {
            Err(get_error())
        } else {
            Ok(Chunk {
    ...
    pub fn set_volume(&mut self, volume: i32) -> i32 {
        unsafe { mixed::Mix_VolumeChunk(self.raw, volume as c_int) as i32 }
    }

    /// current volume for the chunk.
    pub fn get_volume(&self) -> i32 {
        unsafe { mixed::Mix_VolumeChunk(self.raw, -1) as i32 }
    }
}
...
impl<'a> LoaderRWops<'a> for RWops<'a> {
    /// Load src for use as a sample.
    fn load_wav(&self) -> Result<Chunk, String> {
        let raw = unsafe { mixed::Mix_LoadWAV_RW(self.raw(), 0) };
        if raw.is_null() {
            Err(get_error())
        } else {
            Ok(Chunk {
    ...
    fn load_music(&self) -> Result<Music<'a>, String> {
        let raw = unsafe { mixed::Mix_LoadMUS_RW(self.raw(), 0) };
        if raw.is_null() {
            Err(get_error())
        } else {
            Ok(Music {
...
pub enum Fading {
    NoFading = mixed::Mix_Fading_MIX_NO_FADING as i32,
    FadingOut = mixed::Mix_Fading_MIX_FADING_OUT as i32,
    FadingIn = mixed::Mix_Fading_MIX_FADING_IN as i32,
}
...
pub fn allocate_channels(numchans: i32) -> i32 {
    unsafe { mixed::Mix_AllocateChannels(numchans as c_int) as i32 }
}
...
pub fn set_channel_finished(f: fn(Channel)) {
    unsafe {
        ...
        mixed::Mix_ChannelFinished(Some(c_channel_finished_callback as extern "C" fn(ch: c_int)));
    }
}
...
pub fn unset_channel_finished() {
    unsafe {
        mixed::Mix_ChannelFinished(None);
...
impl Channel {
    ...
    pub fn set_volume(self, volume: i32) -> i32 {
        let Channel(ch) = self;
        unsafe { mixed::Mix_Volume(ch as c_int, volume as c_int) as i32 }
    }

    /// Returns the channels volume on scale of 0 to 128.
    pub fn get_volume(self) -> i32 {
        let Channel(ch) = self;
        unsafe { mixed::Mix_Volume(ch as c_int, -1) as i32 }
    }
    ...
    pub fn play_timed(self, chunk: &Chunk, loops: i32, ticks: i32) -> Result<Channel, String> {
        let Channel(ch) = self;
        let ret = unsafe {
            mixed::Mix_PlayChannelTimed(ch as c_int, chunk.raw, loops as c_int, ticks as c_int)
        };
        if ret == -1 {
            Err(get_error())
        } else {
    ...
    pub fn fade_in_timed(self,
        ...
        let Channel(ch) = self;
        let ret = unsafe {
            mixed::Mix_FadeInChannelTimed(ch as c_int,
                                        chunk.raw,
                                        loops as c_int,
                                        ms as c_int,
                                        ticks as c_int)
    ...
    pub fn halt(self) {
        let Channel(ch) = self;
        unsafe {
            mixed::Mix_HaltChannel(ch as c_int);
        }
    }
    ...
    pub fn expire(self, ticks: i32) -> i32 {
        let Channel(ch) = self;
        unsafe { mixed::Mix_ExpireChannel(ch as c_int, ticks as c_int) as i32 }
    }

    /// Gradually fade out which channel over ms milliseconds starting from now.
    pub fn fade_out(self, ms: i32) -> i32 {
        let Channel(ch) = self;
        unsafe { mixed::Mix_FadeOutChannel(ch as c_int, ms as c_int) as i32 }
    }

    /// if channel is playing, or not.
    pub fn is_playing(self) -> bool {
        let Channel(ch) = self;
        unsafe { mixed::Mix_Playing(ch as c_int) != 0 }
    }

    ///  if channel is paused, or not.
    pub fn is_paused(self) -> bool {
        let Channel(ch) = self;
        unsafe { mixed::Mix_Paused(ch as c_int) != 0 }
    }

    /// if channel is fading in, out, or not
    pub fn get_fading(self) -> Fading {
        let Channel(ch) = self;
        let ret = unsafe { mixed::Mix_FadingChannel(ch as c_int) as c_uint };
        match ret {
            mixed::Mix_Fading_MIX_FADING_OUT    => Fading::FadingOut,
            mixed::Mix_Fading_MIX_FADING_IN     => Fading::FadingIn,
            mixed::Mix_Fading_MIX_NO_FADING | _ => Fading::NoFading
        }
    }
    ...
    pub fn get_chunk(self) -> Option<Chunk> {
        let Channel(ch) = self;
        let raw = unsafe { mixed::Mix_GetChunk(ch as c_int) };
        if raw.is_null() {
            None
        } else {
            Some(Chunk {
    ...
    pub fn unregister_all_effects(self) -> Result<(), String> {
        let Channel(ch) = self;
        let ret = unsafe { mixed::Mix_UnregisterAllEffects(ch as c_int) };
        if ret == 0 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn set_panning(self, left: u8, right: u8) -> Result<(), String> {
        let Channel(ch) = self;
        let ret = unsafe { mixed::Mix_SetPanning(ch as c_int, left, right) };
        if ret == 0 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn unset_panning(self) -> Result<(), String> {
        let Channel(ch) = self;
        let ret = unsafe { mixed::Mix_SetPanning(ch as c_int, 255, 255) };
        if ret == 0 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn set_distance(self, distance: u8) -> Result<(), String> {
        let Channel(ch) = self;
        let ret = unsafe { mixed::Mix_SetDistance(ch as c_int, distance) };
        if ret == 0 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn unset_distance(self) -> Result<(), String> {
        let Channel(ch) = self;
        let ret = unsafe { mixed::Mix_SetDistance(ch as c_int, 0) };
        if ret == 0 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn set_position(self, angle: i16, distance: u8) -> Result<(), String> {
        let Channel(ch) = self;
        let ret = unsafe { mixed::Mix_SetPosition(ch as c_int, angle, distance) };
        if ret == 0 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn unset_position(self) -> Result<(), String> {
        let Channel(ch) = self;
        let ret = unsafe { mixed::Mix_SetPosition(ch as c_int, 0, 0) };
        if ret == 0 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn set_reverse_stereo(self, flip: bool) -> Result<(), String> {
        let Channel(ch) = self;
        let ret = unsafe { mixed::Mix_SetReverseStereo(ch as c_int, flip as c_int) };
        if ret == 0 {
            Err(get_error())
        } else {
            Ok(())
...
pub fn reserve_channels(num: i32) -> i32 {
    unsafe { mixed::Mix_ReserveChannels(num as c_int) as i32 }
}
...
impl Group {
    ...
    pub fn add_channels_range(self, from: i32, to: i32) -> i32 {
        let Group(g) = self;
        unsafe { mixed::Mix_GroupChannels(from as c_int, to as c_int, g as c_int) as i32 }
    }
    ...
    pub fn add_channel(self, Channel(ch): Channel) -> bool {
        let Group(g) = self;
        unsafe { mixed::Mix_GroupChannel(ch as c_int, g as c_int) == 1 }
    }

    /// Count the number of channels in group
    pub fn count(self) -> i32 {
        let Group(g) = self;
        unsafe { mixed::Mix_GroupCount(g as c_int) as i32 }
    }

    /// Find the first available (not playing) channel in group
    pub fn find_available(self) -> Option<Channel> {
        let Group(g) = self;
        let ret = unsafe { mixed::Mix_GroupAvailable(g as c_int) as i32 };
        if ret == -1 {
            None
        } else {
            Some(Channel(ret))
    ...
    pub fn find_oldest(self) -> Option<Channel> {
        let Group(g) = self;
        let ret = unsafe { mixed::Mix_GroupOldest(g as c_int) as i32 };
        if ret == -1 {
            None
        } else {
            Some(Channel(ret))
    ...
    pub fn find_newest(self) -> Option<Channel> {
        let Group(g) = self;
        let ret = unsafe { mixed::Mix_GroupNewer(g as c_int) as i32 };
        if ret == -1 {
            None
        } else {
            Some(Channel(ret))
    ...
    pub fn fade_out(self, ms: i32) -> i32 {
        let Group(g) = self;
        unsafe { mixed::Mix_FadeOutGroup(g as c_int, ms as c_int) as i32 }
    }
    ...
    pub fn halt(self) {
        let Group(g) = self;
        unsafe {
            mixed::Mix_HaltGroup(g as c_int);
        }
    }
}
...
pub fn get_music_decoders_number() -> i32 {
    unsafe { mixed::Mix_GetNumMusicDecoders() as i32 }
}

/// Get the name of the indexed music decoder.
pub fn get_music_decoder(index: i32) -> String {
    unsafe {
        let name = mixed::Mix_GetMusicDecoder(index as c_int);
        from_utf8(CStr::from_ptr(name).to_bytes()).unwrap().to_owned()
    }
}
...
pub enum MusicType {
    MusicNone = mixed::Mix_MusicType_MUS_NONE as i32,
    MusicCmd = mixed::Mix_MusicType_MUS_CMD as i32,
    MusicWav = mixed::Mix_MusicType_MUS_WAV as i32,
    MusicMod = mixed::Mix_MusicType_MUS_MOD as i32,
    MusicMid = mixed::Mix_MusicType_MUS_MID as i32,
    MusicOgg = mixed::Mix_MusicType_MUS_OGG as i32,
    MusicMp3 = mixed::Mix_MusicType_MUS_MP3 as i32,
    MusicMp3Mad = mixed::Mix_MusicType_MUS_MP3_MAD_UNUSED as i32,
    MusicFlac = mixed::Mix_MusicType_MUS_FLAC as i32,
    MusicModPlug = mixed::Mix_MusicType_MUS_MODPLUG_UNUSED as i32,
}
...
impl<'a> Drop for Music<'a> {
    fn drop(&mut self) {
        if self.owned {
            unsafe { mixed::Mix_FreeMusic(self.raw) };
        }
    }
}
...
impl<'a> Music<'a> {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Music<'static>, String> {
        let raw = unsafe {
            ...
            mixed::Mix_LoadMUS(c_path.as_ptr())
        };
        if raw.is_null() {
            Err(get_error())
        } else {
    ...
    pub fn from_static_bytes(buf: &'static [u8]) -> Result<Music<'static>, String> {
        ...
        let raw = unsafe { mixed::Mix_LoadMUS_RW(rw, 0) };
        if raw.is_null() {
            Err(get_error())
        } else {
            Ok(Music {
    ...
    pub fn get_type(&self) -> MusicType {
        let ret = unsafe { mixed::Mix_GetMusicType(self.raw) as i32 } as c_uint;
        match ret {
            mixed::Mix_MusicType_MUS_CMD      => MusicType::MusicCmd,
            mixed::Mix_MusicType_MUS_WAV      => MusicType::MusicWav,
            mixed::Mix_MusicType_MUS_MOD      => MusicType::MusicMod,
            mixed::Mix_MusicType_MUS_MID      => MusicType::MusicMid,
            mixed::Mix_MusicType_MUS_OGG      => MusicType::MusicOgg,
            mixed::Mix_MusicType_MUS_MP3      => MusicType::MusicMp3,
            mixed::Mix_MusicType_MUS_MP3_MAD_UNUSED  => MusicType::MusicMp3Mad,
            mixed::Mix_MusicType_MUS_FLAC     => MusicType::MusicFlac,
            mixed::Mix_MusicType_MUS_MODPLUG_UNUSED  => MusicType::MusicModPlug,
            mixed::Mix_MusicType_MUS_NONE | _ => MusicType::MusicNone
        }
    }

    /// Play the loaded music loop times through from start to finish.
    pub fn play(&self, loops: i32) -> Result<(), String> {
        let ret = unsafe { mixed::Mix_PlayMusic(self.raw, loops as c_int) };
        if ret == -1 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn fade_in(&self, loops: i32, ms: i32) -> Result<(), String> {
        let ret = unsafe { mixed::Mix_FadeInMusic(self.raw, loops as c_int, ms as c_int) };
        if ret == -1 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn fade_in_from_pos(&self, loops: i32, ms: i32, position: f64) -> Result<(), String> {
        let ret = unsafe {
            mixed::Mix_FadeInMusicPos(self.raw, loops as c_int, ms as c_int, position as c_double)
        };
        if ret == -1 {
            Err(get_error())
        } else {
    ...
    pub fn get_volume() -> i32 {
        unsafe { mixed::Mix_VolumeMusic(-1) as i32 }
    }
    ...
    pub fn set_volume(volume: i32) {
        ...
        let _ = unsafe { mixed::Mix_VolumeMusic(volume as c_int) as i32 };
    }

    /// Pause the music playback.
    pub fn pause() {
        unsafe {
            mixed::Mix_PauseMusic();
        }
    }
    ...
    pub fn rewind() {
        unsafe {
            mixed::Mix_RewindMusic();
        }
    }

    /// Set the position of the currently playing music.
    pub fn set_pos(position: f64) -> Result<(), String> {
        let ret = unsafe { mixed::Mix_SetMusicPosition(position as c_double) };
        if ret == -1 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn set_command(command: &str) -> Result<(), String> {
        let ret = unsafe {
            ...
            mixed::Mix_SetMusicCMD(c_command.as_ptr())
        };
        if ret == -1 {
            Err(get_error())
        } else {
    ...
    pub fn fade_out(ms: i32) -> Result<(), String> {
        let ret = unsafe { mixed::Mix_FadeOutMusic(ms as c_int) };
        if ret == -1 {
            Err(get_error())
        } else {
            Ok(())
    ...
    pub fn hook_finished(f: fn()) {
        unsafe {
            ...
            mixed::Mix_HookMusicFinished(Some(c_music_finished_hook as extern "C" fn()));
        }
    }
    ...
    pub fn unhook_finished() {
        unsafe {
            mixed::Mix_HookMusicFinished(None);
    ...
    pub fn is_playing() -> bool {
        unsafe { mixed::Mix_PlayingMusic() == 1 }
    }

    /// If music is paused, or not.
    pub fn is_paused() -> bool {
        unsafe { mixed::Mix_PausedMusic() == 1 }
    }

    /// If music is fading, or not.
    pub fn get_fading() -> Fading {
        let ret = unsafe { mixed::Mix_FadingMusic() as i32 } as c_uint;
        match ret {
            mixed::Mix_Fading_MIX_FADING_OUT    => Fading::FadingOut,
            mixed::Mix_Fading_MIX_FADING_IN     => Fading::FadingIn,
            mixed::Mix_Fading_MIX_NO_FADING | _ => Fading::NoFading
